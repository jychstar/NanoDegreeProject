...
def play_round(cpu_agent, test_agents, win_counts, num_matches):
    ...
    for _ in range(num_matches):
        ...
        for game in games:
            winner, _, termination = game.play(time_limit=TIME_LIMIT) # real thing happens here
            win_counts[winner] += 1
...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    total_wins = {agent.player: 0 for agent in test_agents} # initialize dic
    total_timeouts = 0.
    total_forfeits = 0.
    ...
    for idx, agent in enumerate(cpu_agents):
        wins = {key: 0 for (key, value) in test_agents} # initialize dic
        wins[agent.player] = 0

        print("{!s:^9}{:^13}".format(idx + 1, agent.name), end="", flush=True)

        counts = play_round(agent, test_agents, wins, num_matches) # each round
        total_timeouts += counts[0]
        total_forfeits += counts[1]
        total_wins = update(total_wins, wins) # update win statistics
        _total = 2 * num_matches
        round_totals = sum([[wins[agent.player], _total - wins[agent.player]]
                            for agent in test_agents], [])  # test agent win/loss
        print(' ' + ' '.join([
            '{:^5}| {:^5}'.format(
