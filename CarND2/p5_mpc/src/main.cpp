        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    // incorporate latency
                    int latency = 100 ;  // millisecond
                    double dt = double (latency) /1000;  // s
                    psi = 0;
                    double x0 = 0 + v * cos (psi) * dt;
                    double y0 = 0 +v * sin (psi) * dt;
                    double psi0 = psi + v / mpc.lf * delta0 * dt;
                    double v0 = v + a0 * dt;

                    double slope = coeffs[1] + coeffs[2] * x0 * 2 +  coeffs[3] * x0 * x0 * 3;
                    //double slope = coeffs[1]; // due to x0 = 0
