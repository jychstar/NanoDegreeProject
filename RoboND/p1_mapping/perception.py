...
def perception_step(Rover):
    ...
    binary_nav = color_thresh(warped,rgb_thresh=(160, 160, 160))
    binary_rock = find_rocks(warped)
    binary_obs = np.absolute(np.float32(binary_nav)-1) * mask   
    
    # 4) Update Rover.vision_image (this will be displayed on left side of screen)
        # Example: Rover.vision_image[:,:,0] = obstacle color-thresholded binary image
        #          Rover.vision_image[:,:,1] = rock_sample color-thresholded binary image
        #          Rover.vision_image[:,:,2] = navigable terrain color-thresholded binary image
    Rover.vision_image[:,:,0] = binary_obs * 255
    ...
    xpix, ypix = rover_coords(binary_nav)
    x_world, y_world = pix_to_world(xpix, ypix, rover_xpos, 
                                rover_ypos, rover_yaw, 
                                Rover.worldmap.shape[0], 10)
    obsxpix,obsypix = rover_coords(binary_obs)
    obs_x_world,obs_y_world = pix_to_world(obsxpix, obsypix, rover_xpos, 
                                rover_ypos, rover_yaw, 
                                Rover.worldmap.shape[0], 10)
    ...
    if binary_rock.any():
        rock_x,rock_y = rover_coords(binary_rock)
